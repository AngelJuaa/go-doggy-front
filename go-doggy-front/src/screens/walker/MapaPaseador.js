...
<body>
  ...
  const finalizarServicioAsync = async (servicioId) => {
    ...
    try {
      ...
      const mascota = response.mascota_nombre || "Mascota";
      const tarifaBase = Number(response.tarifa_base_hora) || 0;
      const duracion = Math.ceil(Math.max(cronometroSegundos, 0) / 60) || Number(response.duracion_minutos) || 0;
      const rawTotal = Number(response.costo_total) || 0;
      const totalCobro = rawTotal > 0
        ? rawTotal
        : tarifaBase > 0 && duracion > 0
        ? Number(((tarifaBase * duracion) / 60).toFixed(2))
        : 0;
      const ganancia = Number((totalCobro * 0.8).toFixed(2));
      const tiempo = `${String(Math.floor(cronometroSegundos / 60)).padStart(2, '0')}:${String(cronometroSegundos % 60).padStart(2, '0')}`;

      Alert.alert("¡Paseo finalizado!", "El paseo ha sido completado.");
      navigation.replace("InicioPaseador", {
        resumenFinalizacion: { mascota, tiempo, totalCobro, ganancia },
      });
    } catch (error) {
      setFinalizandoServicio(false);
  ...
  const finalizarServicio = () => {
    ...
    const confirmar = () => finalizarServicioAsync(servicioActivoId);

    if (Platform.OS === "web") {
      const confirmResult = window.confirm("¿Estás seguro de finalizar este paseo?");
