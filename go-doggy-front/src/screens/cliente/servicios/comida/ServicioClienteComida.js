...
import { styles } from "./ServicioClienteComidaStyles";

export default function Servicio_Cliente_Comida({ route, navigation }) {
  // ========================
  // ESTADOS
  // ========================
  ...
  useFocusEffect(
    useCallback(() => {
      const cargarProductos = async () => {
        try {
          console.log("Cargando productos de comida...");
          setProductos([]);
        } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* TÍTULO */}
      <Text style={styles.titleText}>Comida y Alimentos</Text>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        {productosEjemplo.map((producto, index) => (
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate("Servicio_Detalles_Comida", { producto })
            }
          >
            {/* IMAGEN IZQUIERDA */}
